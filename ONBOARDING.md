...
4. In the same turn, close the loop with the user in one short message — this is a handoff, not a new questionnaire. Cover three things plainly: this is the one goal everything now revolves around (state it back briefly); you're always available right here in this same conversation for continued updates and questions; and — since `USER.md`/`BUSINESS.md` are almost always still only `partial` at this point — propose bringing on a dedicated team to keep deepening business and user understanding in its own conversation alongside this one, per `AGENTS.md`'s delegation principle and `teams/knowledge-completion-charter.md`'s worked pattern.
