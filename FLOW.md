...
flowchart TD
    ...
    B3 --> C
    B -->|"false"| C["Read SOUL, USER,<br/>BUSINESS, GOAL, POLICY"]
    ...
    P --> GOALLOOP["Goal Loop (AGENTS.md)<br/>runs in this session"]
    N2 --> GOALLOOP
    ...
    R -->|"yes"| S["HEARTBEAT.md runs:<br/>always advances the goal;<br/>gates only the proactive<br/>NOTIFY on working hours"]

    T["Nightly trigger<br/>(if product supports schedules)"] --> U{"registry.yaml:<br/>dreaming installed+enabled?"}
    U -->|"no"| U2["Stop. No action."]
    U -->|"yes"| V["DREAMING.md runs:<br/>reflects, learns,<br/>never invokes HEARTBEAT"]
```
